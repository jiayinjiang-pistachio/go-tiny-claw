...
func (m *ApprovalManager) WaitForApproval(taskID string, toolName string, args string, reporter *FeishuReporter) (bool, string) {
	...
	noticMsg := fmt.Sprintf(`⚠️ **高危操作审批请求**
Agent 试图执行以下动作：
- 工具 %s
- 参数 %s
...
👉 请在此消息下方回复 "approve %s" 或 "reject %s" 来决定是否放行。`, toolName, args, taskID, taskID)

	// 注意：因为 Middleware 的签名里没有带 Reporter，我们在 main.go 初始化时必须把 reporter 传进来
	if reporter != nil {
		reporter.sendMsg(noticMsg)
	} else {
		// 回退到终端打印（兼容本地 CLI 模式）
