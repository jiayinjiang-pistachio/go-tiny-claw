...
// 为了演示，这里硬编码了当前市面上几个主流大模型的官方大致定价
var PricingModel = map[string]struct {
	InputPrice  float64
	OutputPrice float64
}{
	"glm-4.5-air": {InputPrice: 0.15, OutputPrice: 0.15}, // 这里假定的大模型价格（每百万 Token，tk）
}
...
type CostTracker struct {
	...
	modelName    string
	session      *engine.Session // 当前所属的会话，用于累加总成本
}

func NewCostTracker(provider provider.LLMProvider, modelName string, session *engine.Session) *CostTracker {
	return &CostTracker{
...
// Gnerate 实现了 LLMProvider 接口！这意味着它可以被无缝注入到 Main Loop
