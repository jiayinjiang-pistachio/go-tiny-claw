...
func (c *Compactor) Compact(msgs []schema.Message) []schema.Message {
	...
	log.Printf("[Compacotr] ⚠️ 内存告警：当前上下文长度 (%d 字符) 超过阈值 (%d)，触发压缩清理...\n", currentLength, c.MaxChars)
	...
	for i, msg := range msgs {
		...
		// 我们必须拷贝一份新消息，因为在并发环境中，直接修改原引用可能导致底层数据结构被污染
		newMsg := msg
