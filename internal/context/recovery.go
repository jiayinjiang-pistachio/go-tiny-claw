...
func (rm *RecoveryManager) AnalyzeAndInject(toolName string, rawError string) string {
	...
	// 我们使用相对稳定的英文系统报错关键字，或者我们自己手写的工具内部固定报错格式
	lowerError := strings.ToLower(rawError)

	switch toolName {
	case "ediit_file":
		// 匹配我们在 07 讲中手写的 fuzzyReplace 的固定报错抛出
		if strings.Contains(rawError, "在文件系统中未找到 old_text") || strings.Contains(rawError, "找不到该代码片段") {
	...
	case "read_file", "write_file":
		...
		if strings.Contains(rawError, "no such file or directory") {
	...
	case "bash":
		if strings.Contains(lowerError, "command not found") {
			...
			hint = "该命令执行被超时强杀。如果他是一个常驻服务（如 server 或 watch），请将其转入后台运行（例如使用 `nohup ... &`），不要阻塞主线程。"
		} else if strings.Contains(lowerError, "syntax error") {
			hint = "Bash 语法错误。请检查引号转义或特殊字符，确保命令在终端可直接运行。"
		}
	}
