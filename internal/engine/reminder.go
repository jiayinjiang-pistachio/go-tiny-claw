...
// 负责在运行时监控上下文，并在模型陷入执念时，动态注入强力打断信息
type ReminderInjector struct {
	// 用于记录连续失败的工具调用指纹（ToolName + Arguments 的Hash）
...
// generateFinerprint 生成工具调用的唯一指纹，用于判断大模型是否在重复相同的动作
...
func (r *ReminderInjector) CheckAndInject(lastToolCall schema.ToolCall, lastResult schema.ToolResult) *schema.Message {
	...
	// 如果工具执行成功，说明 Agent 在这条路上走通了，清空所有失败计数器
...
1. 停止猜测参数。跳出当前的局部思维。
2. 彻底改变你的策略。
3. 如果你确实无法通过系统工具解决当前问题，请直接结束任务向用户说明你需要什么人工帮助，而不是继续盲目消耗 API 资源重试。`, failCount, lastToolCall.Name)
