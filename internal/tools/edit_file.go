...
func fuzzyReplace(originalContent, oldText, newText string) (string, error) {
	...
	trimmeOld := strings.TrimSpace(normalizedOld)
	if trimmeOld != "" {
		count = strings.Count(normalizedContent, trimmeOld)
		if count == 1 {
			// 注意：这里替换时，我们只能替换被 Trim 后的部分，不能直接用 newText 破坏原本的文本
			// 为了保持代码不过于复杂，当触发 L3/L4 时，如果 newText 没有带有正确的缩紧，
			// 可能会导致替换后的代码格式不美观，但这总比直接报错让 Agent 死循环好。
			return strings.Replace(normalizedContent, trimmeOld, newText, 1), nil
		}
	}
...
func (t *EditFileTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	originContent := string(contentBytes)

	// 2. 调用多级模糊替换算法
	newContent, err := fuzzyReplace(originContent, input.OldText, input.NewText)
	if err != nil {
